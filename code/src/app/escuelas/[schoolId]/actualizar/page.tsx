...
export default function ActualizarEscuelaPage({ params }: UpdateFormProps) {
  const [schoolId, setSchoolId] = useState<string>('');
  const [school, setSchool] = useState<typeof nauticalSchools[0] | null>(null);
  ...
  // Unwrap params and find school
  useEffect(() => {
    params.then((p) => {
      const id = p.schoolId;
      setSchoolId(id);
      const foundSchool = nauticalSchools.find((s) => s.id === id);
  ...
  const openCalendly = () => {
    if (typeof window !== 'undefined' && (window as any).Calendly && calendlyLoaded) {
